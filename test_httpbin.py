...
# override Flask().test_client()
class TestClient:
    base_url = os.environ["TEST_BASE_URL"]

    def request(self, method, url: str | bytes, *args, **kwargs):
        if isinstance(url, str):
            _url = self.base_url + url
        else:
            _url = self.base_url.encode('utf-8') + url
        return requests.request(method, _url, *args, **kwargs)

    def __getattr__(self, attr: str):
        if attr in ("get", "head", "post", "patch", "put", "delete", "options"):
...
class HttpbinTestCase(unittest.TestCase):
    ...
    def test_response_headers_simple(self):
        ...
        for verb in supported_verbs:
            ...
            self.assertEqual(response.headers.get('animal'), 'dog')
    ...
    def test_response_headers_multi(self):
        ...
        for verb in supported_verbs:
            ...
            self.assertEqual(response.headers.get('animal'), ', '.join(['dog', 'cat']))
    ...
    def test_get(self):
        ...
        self.assertEqual(data['headers']['Host'], furl(self.app.base_url).netloc)
        # self.assertEqual(data['headers']['Content-Length'], '0')
        self.assertEqual(data['headers']['User-Agent'], 'test')
        # self.assertEqual(data['origin'], None)

        # self.assertEqual(data['url'], 'http://localhost/get')
        self.assertEqual(data['url'], f'{self.app.base_url}/get')
    ...
    def test_anything(self):
        ...
        self.assertEqual(data['headers']['Host'], furl(self.app.base_url).netloc)
        # self.assertEqual(data['headers']['Content-Length'], '0')
        self.assertEqual(data['url'], f'{self.app.base_url}/anything/foo/bar')
    ...
    def test_post_body_text(self):
        ...
        with TemporaryDirectory() as tmp_dir:
            hello_py = Path(tmp_dir).joinpath('hello.py')
            hello_py.write_text(dedent("""\
            #!/usr/bin/env python3
            print("Hello World!")
            """))

            with open(str(hello_py)) as f:
    ...
    def test_post_body_unicode(self):
        ...
        self.assertEqual(response.json()['data'], u'оживлённым')
    ...
    #     response = self.app.post(
    #         '/post',
    #         headers=dict([('Transfer-Encoding', 'chunked'), ('Content-type', 'application/json')]),
    #         data=data,
    #     )
    ...
    def test_set_cors_headers_with_options_verb(self):
        response = self.app.request(url='/get', method='OPTIONS')
        self.assertEqual(
            response.headers.get('Access-Control-Allow-Origin'), '*'
        )
        self.assertEqual(
    ...
    def test_set_cors_allow_headers(self):
        response = self.app.request(url='/get', method='OPTIONS', headers={'Access-Control-Request-Headers': 'X-Test-Header'})
        self.assertEqual(
    ...
    # TODO: environ_base={"REMOTE_ADDR": ...} unclear
    '''
    ...
    def test_drip(self):
        ...
        self.assertEqual(len(response.content), 400)
        self.assertEqual(response.status_code, 200)
    ...
    def test_bytes_with_seed(self):
        ...
        if six.PY3:
            ...
            self.assertEqual(
                response.content, b'\xd8\xc2kB\x82g\xc8Mz\x95'
            )
